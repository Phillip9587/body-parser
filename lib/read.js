...
function read (req, res, next, parse, debug, options) {
  ...
  let stream
  if (options.inflate === false && encoding !== 'identity') {
    next(createError(415, 'content encoding unsupported', {
      encoding: encoding,
    ...
    try {
      stream = req.pipe(createDecompressionStream(encoding, debug))
    } catch (err) {
      next(err)
      return
    }
  ...
  if (verify && charset !== null && !iconv.encodingExists(charset)) {
  ...
  // set raw-body options per request to avoid mutating shared middleware options
  const rawBodyOptions = {
    limit: options.limit,
    length: encoding === 'identity' ? req.headers['content-length'] : undefined,
    encoding: verify ? null : charset
  }
